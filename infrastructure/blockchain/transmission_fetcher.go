...
func (f *transmissionFetcher) findBlockForRound(
	ctx context.Context,
	contractAddress common.Address,
	targetRound uint32,
	isStartRound bool,
...
func (f *transmissionFetcher) estimateBlockForRound(
	ctx context.Context,
	contractAddress common.Address,
	targetRound uint32,
	currentBlock uint64,
...
func (f *transmissionFetcher) fetchTransmissionsInRangeWithRetry(
	ctx context.Context,
	contractAddress common.Address,
...
func (f *transmissionFetcher) splitBlockRangeOptimized(startBlock, endBlock uint64) []entities.BlockRange {
...
func (f *transmissionFetcher) getFromCache(key string) uint64 {
	f.cache.mu.RLock()
...
func (f *transmissionFetcher) putToCache(key string, blockNumber uint64) {
	f.cache.mu.Lock()
...
func (f *transmissionFetcher) cleanupCache() {
	now := time.Now()
