...
type OCR2AggregatorService interface {
	...
	GetConfigFromBlock(ctx context.Context, contractAddress common.Address, blockNumber uint64) (*entities.OCR2Config, error)
}
...
type TransmissionFetcher interface {
	...
	FetchByRounds(ctx context.Context, contractAddress common.Address, startRound, endRound uint32) (*entities.TransmissionResult, error)

	// FetchByBlocks fetches transmissions for a range of blocks
	FetchByBlocks(ctx context.Context, contractAddress common.Address, startBlock, endBlock uint64) (*entities.TransmissionResult, error)

	// FetchByTimeRange fetches transmissions for a time range
	FetchByTimeRange(ctx context.Context, contractAddress common.Address, startTime, endTime time.Time) (*entities.TransmissionResult, error)
}
...
type TransmissionWatcher interface {
	...
	GetLatestTransmissions(ctx context.Context, transmitterAddress common.Address, limit int) ([]entities.TransmitterStatus, error)
}
